...
public class ProxyFilterPotentialSource implements ProxyHistoryFilter {
  ...
  public boolean matches(ProxyHttpRequestResponse requestResponse) {
    ...
    if (httpResponse == null
        || httpResponse.statedMimeType() != burp.api.montoya.http.message.MimeType.HTML) {
      return false;
    }
    ...
    for (ParsedHttpParameter params : httpRequest.parameters(HttpParameterType.URL)) {
      PotentialSource ptSource =
          new PotentialSource(params.name(), params.value().toLowerCase(), httpRequest.url());

      if (!this.cspt.checkIfFalsePositive(ptSource)) {
        valid = true;
        this.paramValueLookup
            .computeIfAbsent(ptSource.paramValue, x -> new HashSet<>())
            .add(ptSource);
      }
    }
