...
public class ClientSidePathTraversalForm {
  ...
  private final DefaultTableModel resultSourceTableModel =
      new DefaultTableModel(new String[] {"Param Name", "URL"}, 0);
  private final DefaultTableModel resultSinkTableModel =
      new DefaultTableModel(new String[] {"Method", "URL"}, 0);
  ...
  public ClientSidePathTraversalForm(ClientSidePathTraversal cspt) {
    ...
    exportSourcesWithCanaryButton.addActionListener(
        e -> {
    ...
    generateNewCanaryValueButton.addActionListener(
        e -> {
          cspt.setCanary(cspt.generateCanaryToken());
          canaryTokenValue.setText(cspt.getCanary());
        });

    copyCanaryValueButton.addActionListener(
        e ->
            Toolkit.getDefaultToolkit()
                .getSystemClipboard()
                .setContents(new StringSelection(cspt.getCanary()), null));
  ...
  public void displayResults(CSPTScannerTask task) {
    ...
    resultsList.addListSelectionListener(
        e -> {
          ...
          displaySourcesAndSinks(
              this.currentTask.getParamValueLookup(),
              this.currentTask.getPathLookup(),
  ...
  public void displaySources(
      Map<String, Set<PotentialSource>> paramValueLookup, String paramValue) {
  ...
  private void createContextualMenusSinks(ClientSidePathTraversal cspt) {
    ...
    resultSinkTable.addMouseListener(
        new MouseAdapter() {
          @Override
    ...
    findLaxSinks.addActionListener(
        e -> {
          String method =
              resultSinkTable.getValueAt(resultSinkTable.getSelectedRow(), 0).toString();
          String url = resultSinkTable.getValueAt(resultSinkTable.getSelectedRow(), 1).toString();

          // Send sinks to the organizer
          cspt.getExploitableSinks(
              method, HttpRequest.httpRequestFromUrl(url).httpService().host());
        });
  ...
  private void createContextualMenusSources(ClientSidePathTraversal cspt) {
    ...
    resultSourceTable.addMouseListener(
        new MouseAdapter() {
          @Override
          public void mouseReleased(MouseEvent e) {
    ...
    copyURLWithCanary.addActionListener(
        e -> {
          String url =
              resultSourceTable.getValueAt(resultSourceTable.getSelectedRow(), 1).toString();
          String param =
              resultSourceTable.getValueAt(resultSourceTable.getSelectedRow(), 0).toString();
          String urlWithCanary = cspt.replaceParamWithCanary(param, url);

          Toolkit.getDefaultToolkit()
              .getSystemClipboard()
              .setContents(new StringSelection(urlWithCanary), null);
        });
    ...
    falsePositiveParam.addActionListener(
        e -> {
          cspt.addFalsePositive(
              resultSourceTable.getValueAt(resultSourceTable.getSelectedRow(), 0).toString(), ".*");
        });
    popupMenu.add(falsePositiveParam);

    falsePositiveParamURL.addActionListener(
        e -> {
          cspt.addFalsePositive(
              resultSourceTable.getValueAt(resultSourceTable.getSelectedRow(), 0).toString(),
              Pattern.quote(
